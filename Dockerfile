...
ENV INTERNAL_API_URL=http://127.0.0.1:3160
...
HEALTHCHECK --interval=60s --timeout=15s --start-period=90s --retries=5 \
  CMD node -e "const http=require('http');const req=http.get('http://127.0.0.1:3160/api/health',{timeout:10000},(r)=>{r.on('data',()=>{});r.on('end',()=>process.exit(r.statusCode===200?0:1))});req.on('error',()=>process.exit(1));req.on('timeout',()=>{req.destroy();process.exit(1)});setTimeout(()=>{req.destroy();process.exit(1)},10000)"
