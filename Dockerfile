...
HEALTHCHECK --interval=60s --timeout=15s --start-period=90s --retries=5 \
  CMD node -e "require('http').get('http://127.0.0.1:3160/api/health', {timeout: 12000}, (res) => { process.exit(res.statusCode === 200 ? 0 : 1) }).on('error', () => process.exit(1)).on('timeout', () => process.exit(1))"
